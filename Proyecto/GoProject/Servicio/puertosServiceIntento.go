...
func main() {
	...
	for {
		...
		//for sw (agarramos mac switch)

		for i := 0; i < qty.NumAp; i++ {
			//if sw mac == devices.UAPs[i].LastUplink.UplinkMac (mac del sw proximo)
			var info InfoPort
		...
		for i := 0; i < qty.NumClients; i++ {
			...
			if clients[i].ApMac != "" {
				...
				//index = find mac in arrsw
				//arrSW[index].clientesap.append(cliente)
			} else {
				arrSw.Ports = append(arrSw.Ports, info)
			}
			//if portclient = algunpuertoap
			//apend al ap
			//else
			//append al sw
